...
The `value` prop is the picto's identifier (if a native picto) or the path to a custom image. The `size` prop is the size of the picto in pixels (default is 16).
