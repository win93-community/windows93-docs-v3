...
Use `on` to listen to events within Plan.
...
Internally this applies `tabIndex: -1` so keyboard capture can work reliably.
...
# Short event reference
Individual events are documented in the Components section.
